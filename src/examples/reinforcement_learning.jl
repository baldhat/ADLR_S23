...
function VtolEnv(;
    ...
    Flyonic.Visualization.set_transform(name, [0.0; 0.0; 0.0] ,QuatRotation(UnitQuaternion(RotY(-pi/2.0)*RotX(pi))));
    ...
    environment = VtolEnv(
        ...
        zeros(T, 3), # x_W
        zeros(T, 3), # v_B
        Matrix(Rotations.UnitQuaternion(RotY(-pi/2.0)*RotX(pi))), # Float64... so T needs to be Float64
        zeros(T, 3), # ω_B
        zeros(T, 3), # wind_W
        T(0.025), # Δt  
    )
