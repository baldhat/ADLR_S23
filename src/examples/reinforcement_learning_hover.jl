...
function VtolEnv(;
    ...
    Flyonic.Visualization.set_transform(name, [0.0; 0.0; 1.0] ,QuatRotation([0 0 -1; 0 1 0; 1 0 0]));
    ...
    environment = VtolEnv(
        ...
        name,
        Array{T}([0.0; 0.0; 1.0]), # x_W
        Array{T}([0,0,0]), # v_B
        Array{T}([0 0 -1; 0 1 0; 1 0 0]), # Float64... so T needs to be Float64
        zeros(T, 3), # ω_B
        zeros(T, 3), # wind_W
        T(0.025), # Δt  
    )
...
function computeReward(env::VtolEnv{A,T}) where {A,T}
    ...
    not_upright_orientation = abs(angle_transformed)*100.0

    not_centered_position = abs(env.state[3])*100.0
    not_correct_height = abs(env.state[4] - 5.0)*50.0
    not_still = (abs(env.state[5]) + abs(env.state[6])) * 50.0
    
    return stay_alive - not_centered_position - not_correct_height - not_upright_orientation - not_still
end
...
function saveModel(t, agent, env)
    ...
    f = joinpath("./src/examples/RL_models_landing/", "vtol_ppo_2_$t.bson")
    @save f model
...
function loadModel()
    f = joinpath("./src/examples/RL_models_landing/", "vtol_ppo_2_200000.bson")
    @load f model
    return model
end
